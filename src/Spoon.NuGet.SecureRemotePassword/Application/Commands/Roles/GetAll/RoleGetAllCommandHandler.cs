...
public sealed class RoleGetAllCommandHandler : IRequestHandler<RoleGetAllCommand, Either<BaseSearchCommandResult<RoleGetAllCommandResult>>>
{
    ...
    {
        var existingRoles = await this._repository.Roles.SearchAsync(
            new DefaultSearchSpecification<Role>(request.Filters.ToList(), new List<Sorting>(), request.Skip, request.Take, request.IncludeDeleted), cancellationToken);

        if (existingRoles.Count == 0)
            return EitherHelper<BaseSearchCommandResult<RoleGetAllCommandResult>>.EntityNotFound(typeof(Role));


        var totalRoles = await this._repository.Roles.CountAsync(new DefaultSearchSpecification<Role>(request.Filters.ToList(), new List<Sorting>(), request.Skip, request.Take, request.IncludeDeleted),
            cancellationToken);
